    ...
    return (
        ...
        <span className='step'></span>
            <div className="book-details" style={{ borderRadius: '40px' }}>
                ...
                <p><strong>رقم البطاقة:</strong> {bookData.account_id}</p>
                <p><strong>نوع الكتاب:</strong> {bookData.document_type}</p>
                <p><strong>رقم الكتاب:</strong> {bookData.book_number}</p>
                <p><strong>تاريخ الكتاب:</strong> {bookData.book_date}</p>
                <p><strong>جهة الكتاب:</strong> {bookData.issuing_authority}</p>
                <p><strong>رقم الوارد:</strong> {bookData.incoming_number}</p>
                <p><strong>تاريخ الوارد:</strong> {bookData.incoming_date}</p>
                <p><strong>ملاحظة:</strong> {bookData.note}</p>
                <form method="POST">
                    <label>اضافة ملاحظة :</label>
                    <input type="text" name="note" />
                </form>
                <div className="buttons">
                    <button className="btn btn-primary"><Link to='/AddExtension' style={{textDecoration:"none", color:"white"}}>اضافة ملحق</Link></button>
